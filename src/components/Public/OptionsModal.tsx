                                ...
                                {group.opcoes.map((item: any) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all ${isSelected ? 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-900/30' : 'bg-slate-50 dark:bg-slate-800/50 border-transparent hover:border-slate-200 dark:hover:border-slate-700'}`}
                                        >
                                            <div className="text-left">
                                                <p className={`text-sm font-bold transition-colors ${isSelected ? 'text-red-700 dark:text-red-400' : 'text-slate-700 dark:text-slate-200'}`}>{item.nomeSubProduto}</p>
                ...
                <div className="p-6 bg-slate-50 dark:bg-slate-950 border-t border-slate-100 dark:border-slate-800 space-y-4 transition-colors">
                    <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                            ...
                            <p className="text-2xl font-black text-red-600 transition-colors">R$ {(unitPrice * quantity).toFixed(2).replace('.', ',')}</p>
                        </div>
                    </div>
                    <button
                        disabled={!isValid}
